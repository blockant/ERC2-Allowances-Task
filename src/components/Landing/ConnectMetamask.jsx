...
const ConnectMetaMask= ({loginMetaMask, setMetaMaskBalance, isMetaMaskConnected, metaMaskAddress, metaMaskBalance})=>{
  ...
  return (
      <>
       {isMetaMaskConnected? <><h1>Metamask connected: Address {metaMaskAddress} and Balance {metaMaskBalance}</h1><br/><ApproveToken/></>: <><Button variant="contained" onClick={connectToMetaMask}>Connect To Metamask</Button></>}
      </>
  );
}
